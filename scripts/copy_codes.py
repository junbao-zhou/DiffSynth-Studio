    ...
    def is_ignored(self, rel_path: str, is_dir: bool) -> bool:
        ...
        for neg, dir_only, anchored, pat in self.rules:
            ...
            if anchored:
                ...
                if self.fnmatch.fnmatch(rel_path_norm, pat) or self.fnmatch.fnmatch(base, pat):
                    matched = True
            if matched:
                ignored = not neg
        return ignored
...
def collect_files(root: Path, matcher, code_only: bool, include_git: bool) -> List[Path]:
    out: List[Path] = []
...
def ensure_remote_dir(dest: str) -> int:
    ...
    return subprocess.call(["ssh", host, "mkdir", "-p", path])
...
def main() -> int:
    ...
    files = collect_files(root, matcher, code_only=not args.all_files, include_git=args.include_git)
